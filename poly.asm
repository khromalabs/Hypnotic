...
lgt@initregs:
	mov  LVGT@loopcounter, cx

	mov   ebx, dut-2		; Upper EBX = Low Pos U
	movzx bx, byte ptr dvt+2	; BX = High Pos V
	mov   ebp, L@DU-2		; Upper EBP = Low Inc U
	mov   bp, word ptr L@DV+2	; BP = High Inc V

	mov   edx, dzt-2		; Upper EDX = Low Pos G
	movzx dx, byte ptr dut+2	; DX = High Pos U
	mov   eax, L@DZ-2		; Upper var2 = Low Inc G
	mov   ax, word ptr L@DU+2	; Low var2 = High Inc U
	mov  LVGT@var2, eax

	mov   ecx, dvt-2		; Upper ECX = Low Pos V
	movzx cx, byte ptr dzt+2	; CX = High Pos G
	mov   eax, L@DV-2		; Upper var3 = Low Inc V
	mov   ax, word ptr L@DZ+2	; Low var3 = High Inc G
	mov   LVGT@var3, eax
...
@innerl:
	mov  al, bl
	mov  ah, dl
	and  eax, 0111111101111111b
@tex0:	mov  al, byte ptr @tex0[eax]
	mov  ah, cl
	mov  al, light_table[eax]
	mov  [edi], al

	shr  si, 1			; restore carry

	adc  ebx, ebp
	adc  edx, LVGT@var2
	adc  ecx, LVGT@var3

	adc  si, si			; save carry

	mov  al, bl
	mov  ah, dl
	and  eax, 0111111101111111b
@tex1:	mov  al, byte ptr @tex1[eax]
	mov  ah, cl
	mov  al, light_table[eax]
	mov  [edi+80], al
