...
p1@2:	cmp  word ptr DTV@R21_X+2, 0
	je   clear@2@2
...
;ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼
lineizq:cmp  word ptr DTV@R10_X+2, 0                                       
	je   short clear@2@1
	mov  bx, word ptr DTV@R10_X+2
	mov  eax, DTV@R10_Y
	@cdq
...
p2@2:	cmp  word ptr DTV@R21_X+2, 0
	je   short clear@2@2
	mov  bx, word ptr DTV@R21_X+2
	mov  eax, DTV@R21_Y
	@cdq
...
;± PROCEDIMIENTO: ³ LVER                                                     ±
;±----------------³----------------------------------------------------------±
;± FUNCION:       ³ Crea una l¡nea vertical mapeada                          ±
;±----------------³----------------------------------------------------------±
...
innerl: mov  si, bx
	...
	adc  ebx, ebp
	adc  bh, dl
tex0:	mov  al, byte ptr innerl[esi]
	mov  [edi], al
	...
	adc  ebx, ebp
	adc  bh, dl
tex1:	mov  al, byte ptr  innerl[esi]
	mov  [edi+80], al
	...
	mov  si, bx
tex2:	mov  al, byte ptr innerl[esi]
	mov  [edi], al

finl:	pop ecx ebx edx eax
